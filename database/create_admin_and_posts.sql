...
-- 1. First, create admin user in Authentication > Users
...
-- 3. Insert sample posts using existing categories
WITH admin_profile AS (
    SELECT id FROM profiles WHERE role = 'admin' LIMIT 1
)
INSERT INTO posts (title, description, content, image, category_id, author_id, likes, views, is_published)
SELECT 
    post_data.title,
...
) AS post_data(title, description, content, image, category_name, likes, views)
